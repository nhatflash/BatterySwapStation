...
import com.swd392.BatterySwapStation.domain.repository.IRepositoryManager;
...
public class VnPayService implements IVnPayService {
    ...
    public String processIPN(HttpServletRequest request) {
        ...
        SwapTransaction transaction = repositoryManager.getSwapTransactionRepository().findById(transactionId)
    ...
    private boolean isTransactionExists(UUID transactionId) {
        return repositoryManager.getSwapTransactionRepository().existsSwapTransactionById(transactionId);
    }
    ...
    private Payment updateSuccessfulPayment(SwapTransaction transaction) {
        var payment = Payment.builder()
                ...
                .build();
        var savedPayment = repositoryManager.getPaymentRepository().save(payment);
        repositoryManager.getSwapTransactionRepository().save(transaction);
        return savedPayment;
    }
    ...
    private void updateFailedPayment(SwapTransaction transaction) {
        var payment = Payment.builder()
                ...
                .notes("Payment with VnPay.")
                .build();
        repositoryManager.getPaymentRepository().save(payment);
    }
